...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('[酒店渠道] 前端页面传入的参数:', params);
    ...
    if (channelCode) {
      whereConditions.push(`r.ChannelCode LIKE '%${channelCode.replace(/'/g, "''")}%'`);
    }
    const channelName = params.channelName || '';
    if (channelName) {
      whereConditions.push(`r.ChannelName LIKE '%${channelName.replace(/'/g, "''")}%'`);
    }
...
ORDER BY h.HotelCode, r.ChannelCode;
    ...
    console.log('[酒店渠道] 查询结果数量:', results.length);
    ...
    try {
      ...
      console.log('[酒店渠道] 获取酒店选项列表完成，酒店编号:', hotelCodeOptions.length, '酒店名称:', hotelNameOptions.length);
    } catch (error) {
      console.error('[酒店渠道] 获取酒店选项列表失败:', error);
    }
    ...
    console.log('[酒店渠道] 返回给前端的内容:', JSON.stringify(responseData, null, 2));

    return NextResponse.json({ success: true, data: responseData, message: '查询成功' });
  } catch (error) {
    console.error('[酒店渠道] 查询失败:', error);
