...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('[酒店房型] 前端页面传入的参数:', params);
    ...
    if (roomTypeCode) {
      whereConditions.push(`r.RoomTypeCode LIKE '%${roomTypeCode.replace(/'/g, "''")}%'`);
    }
    if (roomTypeName) {
      whereConditions.push(`r.RoomTypeName LIKE '%${roomTypeName.replace(/'/g, "''")}%'`);
    }
...
ORDER BY h.HotelCode, r.RoomTypeCode;
    ...
    console.log('[酒店房型] 查询结果数量:', results.length);
    ...
    try {
      ...
      console.log('[酒店房型] 获取酒店选项列表完成，酒店编号:', hotelCodeOptions.length, '酒店名称:', hotelNameOptions.length);
    } catch (error) {
      console.error('[酒店房型] 获取酒店选项列表失败:', error);
    }
    ...
    console.log('[酒店房型] 返回给前端的内容:', JSON.stringify(responseData, null, 2));

    return NextResponse.json({ success: true, data: responseData, message: '查询成功' });
  } catch (error) {
    console.error('[酒店房型] 查询失败:', error);
