...
export default function PackagePricePage() {
  ...
  const [packageIsDelete, setPackageIsDelete] = useState<string>('');
  ...
  const handleQuery = async (toPage?: number, toPageSize?: number) => {
    try {
      ...
      if (packageCode) params.append('packageCode', packageCode);
      if (packageName) params.append('packageName', packageName);
      if (packageStatus !== '') params.append('packageStatus', packageStatus);
      if (packageIsDelete !== '') params.append('packageIsDelete', packageIsDelete);
      params.append('page', String(currentPage));
      params.append('pageSize', String(currentSize));

      const res = await fetch(`/api/product/hotels/package-price?${params.toString()}`);
  ...
  const selectedConditions = useMemo(() => {
    ...
    if (packageCode) conditions.push(`包价代码: ${packageCode}`);
    if (packageName) conditions.push(`包价名称: ${packageName}`);
    if (packageStatus !== '') conditions.push(`包价状态: ${getStatusDisplay(packageStatus)}`);
    if (packageIsDelete !== '') conditions.push(`包价是否删除: ${getIsDeleteDisplay(packageIsDelete)}`);
    return conditions;
  }, [hotelCode, hotelName, groupCodes, hotelTypes, propertyTypes, pmsTypes, areas, urbanAreas, cities, status, isDelete, packageCode, packageName, packageStatus, packageIsDelete]);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8">
            <div className="flex justify-between items-start">
              <div>
                <h1 className="text-3xl font-bold text-gray-900 mb-2">酒店包价</h1>
                <p className="text-gray-600">查询和管理酒店包价信息</p>
              </div>
              <Link
                href="/product"
          ...
          <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">包价代码</label>
                <Input
                  placeholder="输入包价代码..."
                  value={packageCode}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">包价名称</label>
                <Input
                  placeholder="输入包价名称..."
                  value={packageName}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">包价状态</label>
                <Select
                  allowClear
                  placeholder="选择包价状态"
                  className="w-full"
                  value={packageStatus || undefined}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">包价是否删除</label>
                <Select
                  allowClear
                  placeholder="选择包价是否删除"
                  className="w-full"
                  value={packageIsDelete || undefined}
                  onChange={(val) => setPackageIsDelete(val || '')}
                  options={[
                    { label: '全部', value: '' },
                    { label: '正常', value: '0' },
                    { label: '已删除', value: '1' }
          ...
          <div className="bg-white rounded-lg shadow-sm overflow-hidden">
            <Table
              ...
              rowKey={(record) => {
                ...
                return `${record.包价代码 || ''}-${record.酒店编号 || ''}-${record.包价名称 || ''}`;
              }}
            />
