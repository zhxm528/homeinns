...
export default function RoomTypesPage() {
  ...
  const tableColumns: ColumnsType<any> = useMemo(() => {
    return [
      ...
      },
      {
        ...
        key: '房型状态',
        width: 100,
        render: (text: number | string) => text !== null && text !== undefined ? String(text) : '-',
      },
      {
        title: '房型是否删除',
        dataIndex: '房型是否删除',
  ...
  const handleQuery = async (toPage?: number, toPageSize?: number) => {
    try {
      ...
      const res = await fetch(`/api/product/hotels/room-types?${params.toString()}`);
  ...
  const selectedConditions = useMemo(() => {
    ...
    if (roomTypeName) conditions.push(`房型名称: ${roomTypeName}`);
    return conditions;
  }, [hotelCode, hotelName, groupCodes, hotelTypes, propertyTypes, pmsTypes, areas, urbanAreas, cities, status, isDelete, roomTypeCode, roomTypeName]);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8">
            <div className="flex justify-between items-start">
              <div>
                <h1 className="text-3xl font-bold text-gray-900 mb-2">酒店房型</h1>
                <p className="text-gray-600">查询和管理酒店房型信息</p>
              </div>
              <Link
                href="/product"
          ...
          <div className="bg-white rounded-lg shadow-sm overflow-hidden">
            <Table
              ...
              pagination={false}
              rowKey={(record) => {
                return `${record.房型大类代码 || ''}-${record.房型代码}-${record.酒店编号}`;
              }}
            />
