...
@RestController
@RequestMapping("/api/nj/budget/year")
public class NjBudgetYearTemplateController {
    private static final Logger logger = LoggerFactory.getLogger(NjBudgetYearTemplateController.class);
    ...
    public ResponseEntity<byte[]> getBudgetYearTemplate(@RequestBody NjBudgetYearTemplateDTO dto) {
        try {
            // 打印json格式的入参
            String json = objectMapper.writeValueAsString(dto);
            logger.debug("/api/nj/budget/year/template 入参: {}", json);

            // 读取模板
            ClassPathResource resource = new ClassPathResource("static/download/template/NjBudgetYearTemplate.xlsx");
            ...
            String filename = dto.getHotelCode() + dto.getHotelName() + dto.getYear() + "年度预算.xlsx";
            logger.debug("生成Excel成功 - 文件名: {} ", filename);
     ...
     */
    @PostMapping("/makesql")
    public ResponseEntity<String> processBudgetYearTemplate(
        ...
        try {
            // 构建DTO对象
            NjBudgetYearTemplateDTO dto = new NjBudgetYearTemplateDTO();
            ...
            // 打印json格式的入参
            String json = objectMapper.writeValueAsString(dto);
            logger.debug("/api/nj/budget/year/makesql 入参: {}", json);
            ...
            // 处理Excel数据，获取结果列表
            List<NjBudgetYearTemplateSQL> resultList = processExcelData(hotelCode, year, file);
            
            // 生成SQL语句
            String sqlContent = generateSQLContent(resultList);
            ...
            String fileName = originalFileName != null ? 
                ...
                hotelCode + "_" + hotelName + "_" + year + "_年度预算.sql";
            
            // 保存SQL文件到下载目录
            String downloadPath = System.getProperty("user.home") + "/Downloads/" + fileName;
            saveSQLToFile(sqlContent, downloadPath);
            
            logger.debug("SQL文件已生成: {}", downloadPath);
            
            return ResponseEntity.ok("SQL文件已生成: " + fileName);
        } catch (Exception e) {
    ...
    /**
     * 处理Excel数据，组装NjBudgetYearTemplateSQL对象列表
     * @param hotelCode 酒店编码
     * @param year 年份
     * @param file 前端上传的Excel文件
     * @return 处理后的对象列表
     */
    private List<NjBudgetYearTemplateSQL> processExcelData(String hotelCode, String year, MultipartFile file) {
        ...
        try {
            ...
            for (int rowIndex = 1; rowIndex <= lastRowNum; rowIndex++) {
                ...
                if (row != null) {
                    // 获取B列值（业务类型）
                    Cell cellB = row.getCell(1); // B列索引为1
                    if (cellB != null) {
                        String businessType = getCellValueAsString(cellB);
                        if (businessType != null && !businessType.trim().isEmpty()) {
                            businessTypes.add(businessType);
                            
                            // 获取C列值（月份）
                            Cell cellC = row.getCell(2); // C列索引为2
                            if (cellC != null) {
                                String monthStr = getCellValueAsString(cellC);
                                int month = parseMonth(monthStr);
                                
                                // 获取D列值（数值）
                                Cell cellD = row.getCell(3); // D列索引为3
                                if (cellD != null) {
                                    Double value = getCellValueAsDouble(cellD);
            ...
            for (String businessType : businessTypes) {
                ...
                sqlObj.setHotelCode(hotelCode);
                sqlObj.setBusinessDate(year);
    ...
    private NjBudgetYearTemplateSQL calculateOtherIncome(
        ...
        for (int month = 1; month <= 12; month++) {
            ...
            double otherIncome = totalIncome - guestIncome - diningIncome - propertyIncome - rentIncome;
        ...
        otherIncomeObj.setUpdateTime(currentTime.toString());
    ...
    private NjBudgetYearTemplateSQL calculateGuestProfitRate(
        ...
        guestProfitRateObj.setUpdateTime(currentTime.toString());
    ...
    private NjBudgetYearTemplateSQL calculateDiningProfitRate(
        ...
        diningProfitRateObj.setUpdateTime(currentTime.toString());
    ...
    private NjBudgetYearTemplateSQL createEmptyBusinessObject(
        ...
        obj.setUpdateTime(currentTime.toString());
    ...
    /**
     * 生成SQL内容
     * @param resultList 处理后的数据列表
     * @return SQL内容字符串
     */
    private String generateSQLContent(List<NjBudgetYearTemplateSQL> resultList) {
        StringBuilder sqlContent = new StringBuilder();
        sqlContent.append("INSERT INTO [Report].[dbo].StarBudgetFilling (HotelCode, BusinessDate, BusinessType, BusinessName, Month1, Month2, Month3, Month4, Month5, Month6, Month7, Month8, Month9, Month10, Month11, Month12, UpdateTime) VALUES\n");

        for (int i = 0; i < resultList.size(); i++) {
            NjBudgetYearTemplateSQL sqlObj = resultList.get(i);
            sqlContent.append("('").append(sqlObj.getHotelCode()).append("', '").append(sqlObj.getBusinessDate()).append("', '").append(sqlObj.getBusinessType()).append("', '").append(sqlObj.getBusinessName()).append("', '");
            sqlContent.append(formatDoubleForSQL(sqlObj.getMonth1())).append("', '").append(formatDoubleForSQL(sqlObj.getMonth2())).append("', '").append(formatDoubleForSQL(sqlObj.getMonth3())).append("', '").append(formatDoubleForSQL(sqlObj.getMonth4())).append("', '");
            sqlContent.append(formatDoubleForSQL(sqlObj.getMonth5())).append("', '").append(formatDoubleForSQL(sqlObj.getMonth6())).append("', '").append(formatDoubleForSQL(sqlObj.getMonth7())).append("', '").append(formatDoubleForSQL(sqlObj.getMonth8())).append("', '");
            sqlContent.append(formatDoubleForSQL(sqlObj.getMonth9())).append("', '").append(formatDoubleForSQL(sqlObj.getMonth10())).append("', '").append(formatDoubleForSQL(sqlObj.getMonth11())).append("', '").append(formatDoubleForSQL(sqlObj.getMonth12())).append("', '").append(sqlObj.getUpdateTime()).append("'");
