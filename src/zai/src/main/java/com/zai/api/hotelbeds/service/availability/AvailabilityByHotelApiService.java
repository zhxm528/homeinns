...
@Service
public class AvailabilityByHotelApiService {
    //获取酒店资料 hotel-content-api/1.0/hotels
    //获取酒店价格 hotel-api/1.0/hotels
    //hotel-api/1.0/bookings
    public static AvailabilityByHotelApiService instance;
    ...
    public static void main(String[] args) {
        try {
            ...
            AvailabilityByHotelApiService test = AvailabilityByHotelApiService.getInstance();
            //test.mainFirstStep();
            StringBuffer sb=new StringBuffer();
            test.availabilityByHotel(sb,"743640");// 944761,681977,940468,
    ...
    public AvailabilityByHotelApiService() {
    }
    public static AvailabilityByHotelApiService getInstance() {
        if (instance == null) {
            return new AvailabilityByHotelApiService();
        }
        return instance;
    }
    public void availabilityByHotel(StringBuffer sb,String bookingHotelcode) throws IOException {
        try {
            ...
            endLocalDate=endLocalDate.plusDays(daysToAdd);
            ...
            if(bookingHotelcode!=null){
                // 将字符串转换为LocalDate对象
                // 循环遍历日期，直到结束日期为止
                LocalDate currentLocalDate = startLocalDate;
                while (!currentLocalDate.isAfter(endLocalDate)) {
                    // 获取当前日期的字符串格式
                    String currentDateString = currentLocalDate.format(formatter);
                    // 获取后一天日期并转换为字符串
                    LocalDate nextLocalDate = currentLocalDate.plusDays(1);
                    String nextDateString = nextLocalDate.format(formatter);
                    System.out.println("-------------------------------------------");
                    System.out.println("【"+currentDateString+" 至 "+nextDateString+"】");
                    com.zai.api.hotelbeds.entity.availability.AvailabilityResponse apiResponse = getApiResponse(bookingHotelcode,
                            currentDateString,
                            nextDateString,
                            1);
                    ...
                    if(apiResponse!=null&&apiResponse.getHotels()!=null){
                        if(apiResponse.getHotels().getTotal()==0){
                            ...
                            sb.append("【酒店无价格"+apiResponse.getHotels().getTotal()+"】");
                        }
                        ...
                        if(hb_hotelList!=null){
                            for (AvailabilityHotel hb_hotelObj : hb_hotelList) {
                                ...
                                if(hb_roomList!=null&&hb_roomList.size()>0){
                                    System.out.println("【酒店有价格】【房型" + hb_roomList.size()+"个】");
                                    sb.append("【酒店有价格】【房型" + hb_roomList.size()+"个】");
                                }
                                if(hb_roomList!=null) {
                                        ...
                                        if(hb_rateList!=null) {
                                            for (AvailabilityRate hb_rateObj : hb_rateList) {
                                                if(hb_rateObj.getRateType()!=null&&"BOOKABLE".equalsIgnoreCase(hb_rateObj.getRateType())){
                                                    System.out.println("【"+hb_rateObj.getRateType()+"】【RateKey: " +  hb_rateObj.getRateKey()+"】");
                                                }else{
                                                    System.out.println("【"+hb_rateObj.getRateType()+"】【RateKey: " +  hb_rateObj.getRateKey()+"】");
                                                }
                                            }
...
//                InitiateHotelApiService initiateHotelApiService = InitiateHotelApiService.getInstance();
//                // 初始化每日价格
//                initiateHotelApiService.initiateMain(accountName,bookingHotelcode);
    ...
    public AvailabilityResponse getApiResponse(String bookingHotelcode,
        ...
        try {
            ...
            //requestJson.addProperty("sourceMarket","HK"); //sourceMarket: "CN"
