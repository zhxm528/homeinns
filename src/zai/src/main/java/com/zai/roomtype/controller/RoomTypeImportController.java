...
@RestController
@RequestMapping("/api/roomtype/import")
public class RoomTypeImportController {
    private static final Logger logger = LoggerFactory.getLogger(RoomTypeImportController.class);
    ...
    public ResponseEntity<byte[]> downloadHotelImportTemplate(@PathVariable String chainId) {
        ...
        try {
            ...
            // 读取模板
            ClassPathResource resource = new ClassPathResource("static/download/template/RoomTypeImportSimpleTemplate.xlsx");
            if (!resource.exists()) {
                logger.error("模板文件不存在: static/download/template/RoomTypeImportSimpleTemplate.xlsx");
                return ResponseEntity.notFound().build();
            }
            ...
            String filename = chainName + "_房型列表.xlsx";
     ...
     */
    @PostMapping("/parse")
    public ResponseEntity<BaseResponse> parseRoomTypeImportExcel(@RequestParam("chainId") String chainId,
