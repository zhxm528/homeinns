...
interface ImportResponse {
  ...
  message: string;
  data: {
    importedRoomTypes: RoomTypeData[];
    duplicateRoomTypes: RoomTypeData[];
    errorRoomTypes: RoomTypeData[];
    totalCount: number;
...
const RoomTypeImport: React.FC = () => {
  const [selectedChainId, setSelectedChainId] = useState<string>('');
  const [selectedChainName, setSelectedChainName] = useState<string>('');
  const [importResults, setImportResults] = useState<RoomTypeData[]>([]);
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      console.log('=== 房型导入请求信息 ===');
      console.log('请求URL:', '/api/roomtype/import/parse');
      ...
      const response = await request.post('/api/roomtype/import/parse', formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });

      console.log('=== 房型导入响应信息 ===');
      console.log('响应数据:');
      ...
      if (response.data.success) {
        ...
        // 合并所有房型数据用于显示
        const allRoomTypes: RoomTypeData[] = [
          ...resultData.importedRoomTypes.map((roomType: RoomTypeData) => ({ ...roomType, resultType: 'imported' })),
          ...resultData.duplicateRoomTypes.map((roomType: RoomTypeData) => ({ ...roomType, resultType: 'duplicate' })),
          ...resultData.errorRoomTypes.map((roomType: RoomTypeData) => ({ ...roomType, resultType: 'error' }))
        ];
        
        setImportResults(allRoomTypes);
  ...
  const handleDownloadTemplate = async () => {
    ...
    try {
      ...
      console.log('请求URL:', `/api/roomtype/import/download/${selectedChainId}`);
      console.log('请求方法:', 'GET');
      console.log('集团ID:', selectedChainId);
      console.log('========================');

      const response = await request.get(`/api/roomtype/import/download/${selectedChainId}`, {
      ...
      // 使用集团名称作为文件名
      const fileName = `${chainName}_房型列表.xlsx`;
    ...
    <TokenCheck>
      <div className="p-6">
        ...
        <div className="bg-white rounded-lg shadow p-6 mb-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">房型导入</h2>
