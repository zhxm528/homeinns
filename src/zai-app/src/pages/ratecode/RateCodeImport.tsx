...
interface ImportResponse {
  ...
  message: string;
  data: {
    importedRateCodes: RateCodeData[];
    duplicateRateCodes: RateCodeData[];
    errorRateCodes: RateCodeData[];
    totalCount: number;
...
const RateCodeImport: React.FC = () => {
  const [selectedChainId, setSelectedChainId] = useState<string>('');
  const [selectedChainName, setSelectedChainName] = useState<string>('');
  const [importResults, setImportResults] = useState<RateCodeData[]>([]);
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const response = await request.post('/api/ratecode/import/parse', formData, {
        headers: {
      ...
      if (response.data.success) {
        ...
        // 合并所有房价码数据用于显示
        const allRateCodes: RateCodeData[] = [
          ...resultData.importedRateCodes.map((rateCode: RateCodeData) => ({ ...rateCode, resultType: 'imported' })),
          ...resultData.duplicateRateCodes.map((rateCode: RateCodeData) => ({ ...rateCode, resultType: 'duplicate' })),
          ...resultData.errorRateCodes.map((rateCode: RateCodeData) => ({ ...rateCode, resultType: 'error' }))
        ];
        
        setImportResults(allRateCodes);
  ...
  const handleDownloadTemplate = async () => {
    ...
    try {
      ...
      const response = await request.get(`/api/ratecode/import/download/${selectedChainId}`, {
      ...
      // 使用集团名称作为文件名
      const fileName = `${chainName}_房价码列表.xlsx`;
    ...
    <TokenCheck>
      <div className="p-6">
        ...
        <div className="bg-white rounded-lg shadow p-6 mb-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">房价码导入</h2>
