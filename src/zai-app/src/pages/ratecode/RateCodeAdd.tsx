...
const RateCodeAdd: React.FC = () => {
  ...
  useEffect(() => {
    console.log('RateCodeAdd useEffect triggered');
    // 从 localStorage 获取 chainId 和 hotelId
    const userStr = localStorage.getItem('user');
    if (userStr) {
      try {
        const user = JSON.parse(userStr);
        if (user.chainId) {
          console.log('Setting chainId from localStorage:', user.chainId);
          setFormData(prev => ({
            ...prev,
        ...
        if (user.hotelId) {
          console.log('Setting hotelId from localStorage:', user.hotelId);
          setFormData(prev => ({
            ...prev,
  ...
  const validateForm = (): boolean => {
    ...
    const requiredFields: (keyof RateCodeForm)[] = [
      'chain_id', 'hotel_id', 'rate_code', 'rate_code_name', 'market_code',
      'stay_start_date', 'stay_end_date', 'booking_start_date', 'booking_end_date', 'description', 'price_rule_type'
    ];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      try {
        ...
        const res = await request.post('/api/ratecode/add', submitData);
        console.log('新增房价码 - 响应数据:', JSON.stringify(res.data, null, 2));
        showToast(res.data.message || '添加房价码成功', 'success');
        
        setTimeout(() => {
          if (tabContext && tabContext.removeTab) {
            tabContext.removeTab('/api/ratecode/add');
          }
  ...
  // 切换panel展开/收起状态
  const togglePanel = (panelName: 'basicInfo' | 'bookingRules' | 'priceSettings' | 'advancedSettings') => {
    setPanelStates(prev => ({
      ...prev,
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow">
                     ...
                     <Col span={24}>
                       <div>
                         ...
                         <HotelSelect
                           ...
                           onChange={(value: string) => handleInputChange('hotel_id', value)}
                         />
                         {errors.hotel_id && (
                           <p className="mt-1 text-sm text-red-600">{errors.hotel_id}</p>
                         )}
                       </div>
                     </Col>
                     ...
                     <Col xs={24} sm={12}>
                       <div>
                         ...
                         <input
                           ...
                           onChange={(e) => handleInputChange('rate_code', e.target.value)}
                         />
                         {errors.rate_code && (
                           <p className="mt-1 text-sm text-red-600">{errors.rate_code}</p>
                         )}
                       </div>
                     </Col>
          ...
          <div className="mb-8">
           ...
           <div className="mb-8">
                 <div 
                   className="bg-gray-50 px-4 py-3 border-b border-gray-200 cursor-pointer hover:bg-gray-100 transition-colors"
                   onClick={() => togglePanel('priceSettings')}
                 >
                   <div className="flex items-center justify-between">
                     <h3 className="text-lg font-medium text-gray-900">价格设置</h3>
                     <svg 
                       className={`w-5 h-5 text-gray-500 transition-transform ${panelStates.priceSettings ? 'rotate-180' : ''}`}
                       fill="none" 
                       stroke="currentColor" 
                       viewBox="0 0 24 24"
                     >
                    ...
                    <div className="p-6">
                      <Row gutter={[16, 16]}>
                        ...
                        <Col xs={24} sm={12}>
                          <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">
                              价格规则 <span className="text-red-500">*</span>
                            </label>
                            <RateCodePriceRuleType
                              value={formData.price_rule_type}
                              onChange={(value) => handleInputChange('price_rule_type', value)}
                              placeholder="请选择价格规则"
                            />
                            {errors.price_rule_type && (
                              <p className="mt-1 text-sm text-red-600">{errors.price_rule_type}</p>
                            )}
                          </div>
                        </Col>
