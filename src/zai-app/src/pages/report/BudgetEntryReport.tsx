...
const initialData = budgetItems.map((item, i) => {
  ...
  months.forEach((m, j) => {
    if (item === '出租率' || item === '客房利润率' || item === '餐饮利润率' || item === 'GOP率') {
...
const columns = [
  { title: '预算项', dataIndex: 'item', key: 'item', fixed: 'left', width: 120 },
  ...months.map((m, i) => ({
    title: m,
    dataIndex: `m${i + 1}`,
    key: `m${i + 1}`,
  ...
  return (
    <TokenCheck>
      <div className="p-6">
        ...
        <Card style={{ marginTop: '24px' }} title="预算填报" bordered={false}>
          <Table
        ...
        >
          ...
          {modal.visible && (
            <div style={{ marginBottom: 12, fontWeight: 500, fontSize: 16 }}>
              <span style={{ marginRight: 24 }}>预算项：{data.find(row => row.key === modal.rowKey)?.item}</span>
              <span>月份：{(() => {
          ...
          <div style={{ marginBottom: 16 }}>
            <InputNumber
              ...
              value={editValue}
              style={{ width: '100%' }}
              formatter={v => v ? (modal.rowKey !== -1 && ['出租率','客房利润率','餐饮利润率'].includes(data.find(row => row.key === modal.rowKey)?.item) ? Number(v).toLocaleString('en-US') + '%' : Number(v).toLocaleString('en-US')) : ''}
          ...
          <div style={{ maxHeight: 200, overflow: 'auto', marginBottom: 16 }}>
            <Table
              size="small"
              columns={[
                { title: '预算值', dataIndex: 'value', key: 'value', render: (v: number) => v.toLocaleString('en-US') },
