  ...
  useEffect(() => {
    const fetchHotelInfo = async () => {
      ...
      try {
        ...
        const response = await request.get(`/api/hotel/${hotelId}`);
        ...
        if (response.data.success) {
          ...
          setFormData({
            ...
            ownershipType: hotelData.ownershipType || '',
            managementModel: hotelData.managementModel || '',
            managementCompany: hotelData.managementCompany || '',
            brand: hotelData.brand || '',
            region: hotelData.region || '',
            cityArea: hotelData.cityArea || '',
            pmsVersion: hotelData.pmsVersion || '',
            openingDate: hotelData.openingDate || '',
            lastRenovationDate: hotelData.lastRenovationDate || '',
            closureDate: hotelData.closureDate || '',
            totalPhysicalRooms: hotelData.totalPhysicalRooms || 0,
            basePrice: hotelData.basePrice || 0,
            thresholdPrice: hotelData.thresholdPrice || 0,
            breakfastPrice: hotelData.breakfastPrice || 0,
            parkingPrice: hotelData.parkingPrice || 0
          });
        } else {
    ...
    try {
      ...
      if (!formData.chainId) {
        ...
        return;
            ...
            // 构建请求体 - 只包含酒店信息，符合后台接口要求
       const requestBody = {
         hotelId: hotelId,
         chainId: formData.chainId,
         hotelCode: typeof formData.hotelCode === 'string' ? formData.hotelCode.trim() : (formData.hotelCode || ''),
         hotelName: typeof formData.hotelName === 'string' ? formData.hotelName.trim() : (formData.hotelName || ''),
         ...
         country: typeof formData.country === 'string' ? formData.country.trim() : (formData.country || ''),
         status: formData.status,
         contactEmail: typeof formData.contactEmail === 'string' ? formData.contactEmail.trim() : (formData.contactEmail || ''),
         contactPhone: typeof formData.contactPhone === 'string' ? formData.contactPhone.trim() : (formData.contactPhone || ''),
         ownershipType: typeof formData.ownershipType === 'string' ? formData.ownershipType.trim() : (formData.ownershipType || ''),
         managementModel: typeof formData.managementModel === 'string' ? formData.managementModel.trim() : (formData.managementModel || ''),
         managementCompany: typeof formData.managementCompany === 'string' ? formData.managementCompany.trim() : (formData.managementCompany || ''),
         brand: typeof formData.brand === 'string' ? formData.brand.trim() : (formData.brand || ''),
         region: typeof formData.region === 'string' ? formData.region.trim() : (formData.region || ''),
         cityArea: typeof formData.cityArea === 'string' ? formData.cityArea.trim() : (formData.cityArea || ''),
         pmsVersion: typeof formData.pmsVersion === 'string' ? formData.pmsVersion.trim() : (formData.pmsVersion || ''),
         openingDate: typeof formData.openingDate === 'string' ? formData.openingDate.trim() : (formData.openingDate || ''),
         lastRenovationDate: typeof formData.lastRenovationDate === 'string' ? formData.lastRenovationDate.trim() : (formData.lastRenovationDate || ''),
         closureDate: typeof formData.closureDate === 'string' ? formData.closureDate.trim() : (formData.closureDate || ''),
      ...
      if (response.data.success) {
        ...
        Modal.success({
          ...
          content: (
            <div className="text-left">
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">                
                ...
                {hotelData.region && (
                  <>
                    <div className="font-medium">大区：</div>
                    <div>{hotelData.region}</div>
                  </>
                )}
                {hotelData.cityArea && (
                  <>
                    <div className="font-medium">城区：</div>
          ...
          onOk: () => {
            if (tabContext) {
              // 关闭当前tab
              tabContext.removeTab(`/hotel/edit/${hotelId}`);
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center justify-center h-64">
              <div className="text-center">
                ...
                <p className="text-gray-600">加载酒店信息中...</p>
              </div>
            </div>
          </div>
        </div>
